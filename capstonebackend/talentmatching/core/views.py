...
### note: to create views (or "pages"), Django has 2 ways of doing:
### via function based views & class based views.
### in this file, we are using function based views.
### for further implementation, the code can be transformed to class based views,
### so some parts of the code dont need to be repeated.
...
def alljobs(request):
    ...
    return render(request, 'alljobs.html', {'data': store_data})
...
def allapplicants(request):
    ...
    for applicant in data:
        ...
        job = MatchedJobModel.objects.filter(job_id = job_id).values_list("job_name", flat=True)[0]
        applicant_percent = ModelPredictionModel.objects.filter(job_id_id = job_id, applicant_id_id = applicant_id).values_list("applicant_percent", flat=True)[0]
    ...
    return render(request, 'allapplicants.html', {'data': store_data})
...
def viewjob(request, job_id):
    ...
    recruiter_firstname = User.objects.filter(id=job_details.recruiter_id_id).values_list("first_name", flat=True)[0]
    recruiter_lastname = User.objects.filter(id=job_details.recruiter_id_id).values_list("last_name", flat=True)[0]
    recruiter_name = recruiter_firstname + " " + recruiter_lastname
    ...
    for applicant in data:
        ...
        predict_data = ModelPredictionModel.objects.filter(applicant_id_id = id, job_id_id = job_id)[0]
...
def viewapplicant(request, applicant_id):
    ...
    if applicant.reprofile == True:
        ...
        reprofiled_job_name = MatchedJobModel.objects.filter(job_id = reprofiled_job_id).values_list("job_name", flat=True)[0]
    else:
        reprofiled_job_name = "-"
    ...
    for result in predict_data:
        ...
        predict_job_name = MatchedJobModel.objects.filter(job_id = predict_job_id).values_list("job_name", flat=True)[0]
    ...
    for row in applicant_history:
        history_job_name = MatchedJobModel.objects.filter(job_id = row.job_id_id).values_list("job_name", flat=True)[0]
        history_status = row.recruiter_decision
        history_date = row.status_change_date
        recruiter_firstname = User.objects.filter(id=row.recruiter_id_id).values_list("first_name", flat=True)[0]
        recruiter_lastname = User.objects.filter(id=row.recruiter_id_id).values_list("last_name", flat=True)[0]
        history_recruiter_name = recruiter_firstname + " " + recruiter_lastname
...
def template(request):
    # load_model = pickle.load(open("static/model_backend_test.sav", "rb"))
    # file_location = "static/GPIS Executive/"
    # file_storage = FileSystemStorage(location = file_location)
    # file = file_storage.open("Not Suitable - 1.pdf")
    # data = load_model(file)
    # print(data)
